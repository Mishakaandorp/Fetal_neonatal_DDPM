...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, shape, condition_tensors = None):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            # breakpoint()
            # nifti_img_gen[:,:,round(nifti_img_gen.shape[2]/2)]
            # breakpoint()
            # if i > 500:
            #     if i % 100 == 0:
            #         # breakpoint()
            #         plt.figure()
            #         plt.axis('off') 
            #         middle_slice = ndimage.rotate(img[0,0,round(img.shape[4]/2),:,:].cpu(), 180, reshape=True)
            #         flipped_slice = np.flip(middle_slice, axis=1)
            #         plt.imshow(flipped_slice, cmap='gray') 
            #         # imgX = np.flip(img, axis=1)
            #         # breakpoint()

            #         plt.show()
            #         plt.savefig(f'/media/m-ssd4/Misha/Project_synthetic_brain/med-ddpm/Samples_test/newfetal007_very_good_seed_530676/img_{i}.png')
            ...
            #     plt.show()
            #     plt.imshow(ndimage.rotate(img[0,0,round(img.shape[4]/2)].cpu(), 180, reshape=True), cmap='gray') 

            #     plt.savefig(f'/media/m-ssd3/Misha/Project_synthetic_brain/med-ddpm/Samples_test/test_2_good_image/img_good2_{i}.png')
...
class Trainer(object):
    def __init__(
        ...
        best_val = 0.007,
        # best_val = 90,
        results_folder = './results_combined_1602_sz_1000_imsize_160_test_1',
        with_condition = False,
        with_pairwised = False):
        super().__init__()
    ...
    def save_lowest_loss(self, milestone,best_loss):
        data = {
            'step': self.step,
            'model': self.model.state_dict(),
            'ema': self.ema_model.state_dict()
        }
        torch.save(data, str(self.results_folder / f'model-{milestone}-loss-{best_loss}.pt'))
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step % 1000 == 0:
                print(self.opt)
            if self.step > 100000:
                self.opt = Adam(self.model.parameters(), lr=1e-6)
            # print(self.opt)
            # print(self.opt[1])
            # if self.step >5: 
            #     optim.param_groups[0]['lr'] = 1e-8
            ...
            if loss.item() < self.best_val:
                ...
                milestone_lowest_loss = self.step
                print(f'lowest_loss: {self.best_val}')
                print(f'milestone_lowest_loss: {milestone_lowest_loss}')
                
                self.save_lowest_loss(milestone_lowest_loss,self.best_val)
