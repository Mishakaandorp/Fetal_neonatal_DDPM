...
parser.add_argument('-i', '--inputfolder', type=str, default="")
...
parser.add_argument('-w', '--weightfile', type=str, default="")
parser.add_argument('-fseed', '--fix_seed',action='store_true', help='Fix the random seed during image generations')
...
image_3_dir = exportfolder + "/image_3"   
os.makedirs(img_dir, exist_ok=True)
os.makedirs(msk_dir, exist_ok=True)
os.makedirs(image_3_dir, exist_ok=True)

if fix_seed:
    # if fixed seed then ..
    seed = 19479508450263621
    random_seed = np.repeat(seed,1000)
    print(f'Fixed seed for each image generation..')
    print(f'Seed: {seed}')
else:
    # if random seed then ..
    random_seed = torch.randint(0, 128522342544429100,(1000,))
    print(f'Random Seeds for each image generation.. ')
seed = -1
for k, inputfile in enumerate(mask_list):
    ...
    for i, bsize in enumerate(batches):
        if k %1 ==0:
            seed += 1
        torch.manual_seed(random_seed[k])
