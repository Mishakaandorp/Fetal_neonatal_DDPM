...
# For this function you need the original MRI to know to intensity rescaling
...
parser.add_argument('--T2_images', type=str, default='')
parser.add_argument('--SYNH_files_PER_SUB', type=float)

args = parser.parse_args()
base_dataset = args.base_dataset
synthesize_option = args.synthesize_option
T2_images = args.T2_images

SYNH_files_PER_SUB = args.SYNH_files_PER_SUB
...
def map_label_values(label_map):
    data = label_map.get_fdata()
...
label_map_input_dir_synth = f"../generated_MRIs/160_space/{base_dataset}/{synthesize_option}/mask"
image_input_dir_synth = f"../generated_MRIs/160_space/{base_dataset}/{synthesize_option}/image"

# FeTA_multiple
label_map_input_dir = f"../manipulated_labels/{base_dataset}/{synthesize_option}/all_final_labels"
image_input_dir = T2_images

label_map_output_dir_160 = f"../preprocessing_manipulated_labels/{base_dataset}/{synthesize_option}/label_160_space/"
mri_output_dir_160 = f"../preprocessing_manipulated_labels/{base_dataset}/{synthesize_option}/mri_160_space/"

label_map_output_dir_inverted = f"../preprocessing_manipulated_labels/{base_dataset}/{synthesize_option}/label_original_space_inverted/"
# mri_output_dir_inverted = f"../preprocessing_manipulated_labels/{base_dataset}/{synthesize_option}/mri_original_space_inverted/"
mri_output_dir_inverted = f"../generated_MRIs/original_space/{base_dataset}/{synthesize_option}/image"
original_space_label_copy = f"../generated_MRIs/original_space/{base_dataset}/{synthesize_option}/label"


image_output_dir_in_between = f"../preprocessing_manipulated_labels/{base_dataset}/{synthesize_option}/mri_between/"
...
os.makedirs(label_map_output_dir_160, exist_ok=True)
os.makedirs(mri_output_dir_160, exist_ok=True)
# breakpoint()
...
os.makedirs(original_space_label_copy, exist_ok=True)
os.makedirs(mri_output_dir_inverted, exist_ok=True)
...
list_label_map_input_dir_synth = sorted(os.listdir(label_map_input_dir_synth))
list_image_input_dir_synth = sorted(os.listdir(image_input_dir_synth))

# Process all files
print(f'MRIs to be upscaled: {len(list_label_map_input_dir)}')
count = -1
# print(SYNH_files_PER_SUB)

for idx, filename in enumerate(list_label_map_input_dir):
    if filename.endswith(".nii.gz"):
        if idx % SYNH_files_PER_SUB == 0:
            print('True')
            count += 1
        print(f'MRI: {count}')
        print(f'')
        ...
        corresponding_image_files_synth = sorted([f for f in list_image_input_dir_synth if f.startswith(base_filename)])
        corresponding_label_files_synth = sorted([k for k in list_label_map_input_dir_synth if k.startswith(base_filename)])

        # print(f'base_orig_dataset:{base_orig_dataset}')
        print(f'subject: {base_filename}')
        # breakpoint()

        input_image_file = os.path.join(image_input_dir, list_image_input_dir[count])
        input_label_file = os.path.join(label_map_input_dir, list_label_map_input_dir[idx])
        copy_label_file = os.path.join(original_space_label_copy, list_label_map_input_dir[idx])

        # source file and destination path
        original_space_label_copy
        shutil.copy2(input_label_file, copy_label_file)
        # if base_filename == 'sub-CC01140XX11':
        #     breakpoint() 
        input_image_file_synth = os.path.join(image_input_dir_synth, list_image_input_dir_synth[idx])
        input_label_file_synth = os.path.join(label_map_input_dir_synth, list_label_map_input_dir_synth[idx])
        # print(len(corresponding_image_files))
        # print(len(corresponding_label_files))

        # print(len(input_image_file_synth))
        # print(len(input_label_file_synth))


        input_image_file_i = list_image_input_dir[count]
        input_label_file_i = list_label_map_input_dir[idx]

        input_image_file_i_synth = list_image_input_dir_synth[idx]
        input_label_file_i_synth = list_label_map_input_dir_synth[idx]
        # print(input_image_file_i)
        # print(input_label_file_i)

        
        print(f'processing MRI: {input_image_file_i}')
        print(f'processing LABEL: {input_label_file_i}')
        ...
        label_map_synth = nib.load(input_label_file_synth)
        image_synth = nib.load(input_image_file_synth)
        # print(image.shape)

        orig_mri_map = os.path.join(orig_output_dir, input_image_file_i)
        nib.save(image, orig_mri_map)
        # orig_mri_map_synth = os.path.join(orig_output_dir, input_image_file_i)
        ...
        mapped_label_map_nii = map_label_values(label_map)
        ...
        resize_back_rescaled_command = f"c3d '{input_image_file_synth}' -interpolation Linear -resample {rescaled_image.shape[0]}x{rescaled_image.shape[1]}x{rescaled_image.shape[2]}vox -o '{resized_back_rescaled_image_file}'"
        ...
        resize_back_cropped_command = f"c3d '{input_label_file_synth}' -interpolation NearestNeighbor -resample {cropped_label_map.shape[0]}x{cropped_label_map.shape[1]}x{cropped_label_map.shape[2]}vox -o '{resized_back_cropped_label_file}'"
        ...
        uncropped_mri_file = os.path.join(mri_output_dir_inverted, input_label_file_i)
        print(uncropped_mri_file)
        ...
        print(f'processed MRI: {input_image_file_i}')
        print(f'processed LABEL: {input_label_file_i}')
        print(f'')
